...
drop publication coord1_pub_app
drop subscription coord1_sub_from_coord2

create publication coord1_pub_app
for table app_users;

create subscription coord1_sub_from_coord2
connection 'host=citus-coordinator-2.citus port=5432 dbname=postgres user=repl_user password=111'
publication coord2_pub_app
...
select * from master_add_node('citus-worker-1-0.citus-worker-1-headless.citus.svc.cluster.local', 5432);
select * from master_add_node('citus-worker-1-1.citus-worker-1-headless.citus.svc.cluster.local', 5432);
select * from master_add_node('citus-worker-1-2.citus-worker-1-headless.citus.svc.cluster.local', 5432);
